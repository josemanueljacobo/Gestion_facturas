...
export async function POST(request: NextRequest) {
    try {
        ...
        for (const row of results) {
            ...
            // Factusol FRE.xls format - All required columns
        ...
        if (excelData.length === 0) {
            return NextResponse.json(
                { error: 'No hay facturas validadas en el periodo seleccionado para exportar' },
                { status: 400 }
            );
        }

        // Create Excel workbook with explicit column headers
        // Define all column headers A-CT
        ...
        const worksheet = XLSX.utils.json_to_sheet(excelData, {
            ...
            skipHeader: true  // Don't include header row in output
        });
        ...
        // Mark invoices as exported (fixed to update all invoices, not just the first one)
